#include "model_point_BL_RS_DE_LIE_01.h"
...
Model_Point_BL_RS_DE_LIE_01::~Model_Point_BL_RS_DE_LIE_01(){}
Model_Point_BL_RS_DE_LIE_01::Model_Point_BL_RS_DE_LIE_01(vector<string>& row, int rank, Scenario_configuration& scenario_config):Model_Point(row,rank,scenario_config){initialise_mp_from_inforce(row);}
Model_Point_BL_RS_DE_LIE_01::Model_Point_BL_RS_DE_LIE_01(vector<string>& row, int rank,Scenario_configuration& scenario_config, int pv_nb):Model_Point(row,rank,scenario_config,pv_nb){initialise_mp_from_inforce(row);}
Model_Point_BL_RS_DE_LIE_01::Model_Point_BL_RS_DE_LIE_01(Model_Point * mp):Model_Point(mp){}
	
void Model_Point_BL_RS_DE_LIE_01::set_Calc_information_to_Zero()
	{
...
void Model_Point_BL_RS_DE_LIE_01::initialise_mp_from_inforce(vector<string>& row)
	{	
		initialise_mp0(row);

		Person_Id_1_Bl=row[PERSON_ID_1_LIE_RS_DE];
		Person_Id_2_Bl=row[PERSON_ID_2_LIE_RS_DE];
		Init_Charge= atof(row[INIT_CHARGE_LIE_RS_DE].c_str());
		Fund_Name=row[ACCOUNT_CODE_LIE_RS_DE];
		Currency=row[CURRENCY_LIE_RS_DE];
		...
		Issue_Date=row[ISSUE_DATE_LIE_RS_DE];
		NB_Date_Bl=row[NB_DATE_LIE_RS_DE];
		Inv_Date=row[INV_DATE_LIE_RS_DE];
		Def_Period=atoi(row[DEF_PERIOD_LIE_RS_DE].c_str());
		Annuity_Begin=row[ANNUITY_BEGIN_DATE_LIE_RS_DE];
		Prod_Prem_Type=row[PREMIUM_PAYMENT_MODE_LIE_RS_DE];	
		Premium= atof(row[INSURANCE_PREMIUM_LIE_RS_DE].c_str());
		Amount_Inv= atof(row[AMOUNT_INVESTED_LIE_RS_DE].c_str());
		Freq_Mode=row[WITHDRAWAL_MODE_LIE_RS_DE];
		Withdrawal_Rate= atof(row[CONVERSION_RATE_LIE_RS_DE].c_str());
		Inv_Protection= atof(row[DEF_GUAR_LIE_RS_DE].c_str());
		Benefit_Increase_Factor= atof(row[BENEFIT_INCREASE_FACTOR_LIE_RS_DE].c_str());
		Withrawals_Mode_Discount_Bl= atof(row[WITHDRAWAL_MODE_DISCOUNT_LIE_RS_DE].c_str());
		Gmdb_Bl= atof(row[GMDB_LIE_RS_DE].c_str());
		Guarantee= atof(row[CURR_GMWB_LIE_RS_DE].c_str());
		Accum_Withdrawal_Amount= atof(row[ACCUM_WITHDRAWAL_AMOUNT_LIE_RS_DE].c_str());
		...
		Av_After_After= atof(row[ACC_VAL_AFT_AFT_LIE_RS_DE].c_str());
		Base_Capital= atof(row[BASE_CAPITAL_LIE_RS_DE].c_str());
		...
		Fund_Unit_After_After= atof(row[FUND_UNITS_AFT_AFT_LIE_RS_DE].c_str());
		Unit_Price= atof(row[UNIT_PRICE_LIE_RS_DE].c_str());
		AV0=Av_After_After;	

		set_Calc_information_to_Zero();

	}
void Model_Point_BL_RS_DE_LIE_01::initialise_mp_parameters_by_copy(Model_Point * m_p)
	{
