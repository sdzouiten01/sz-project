#ifndef __PROJECTION_AB_DB_SB_H_
#define __PROJECTION_AB_DB_SB_H_
...
#include "Projection classes/Guarantee projection classes/Projection_SB.h"

class Projection_AB_DB_SB : public Projection_AB, public Projection_DB, public Projection_SB {
	
public :
	Projection_AB_DB_SB(const vector<vector <int>>& i_s_a,const vector<vector <int>>& f_s_a,
		...
		Projection_SB(i_s_a, f_s_a, tables, generate_dbg_files, dbg_file_path, t_high_shorter, l_longevity_tables, time_step)
	{
	}
	
	//fucntion of projection for t time
							...
							double mortality_mult, bool shock_av)
	{
		...
		this->project_gmsb_bases_payments_claims(t);
		this->finish_t(t);
