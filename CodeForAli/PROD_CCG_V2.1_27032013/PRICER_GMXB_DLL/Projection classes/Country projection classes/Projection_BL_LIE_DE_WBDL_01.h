...
#include "head_BL_LIE_RS_DE.h"
#include "head_BL.h"
using namespace std;

extern ofstream trace;
class Assumption_BL_LIE_DE_GMWBL_01
{
	public:
		// Assumptions tables------------------------------------------------
...
public:
	...
	double ConstVector[TOTAL_CONSTVECTOR_COLS_LIE_RS_DE][SUBACCT_MAX_PERIOD_MTH +1];
	double MainProjArrayVector[MAIN_PROJ_NUM_COLS_BL][SUBACCT_MAX_PERIOD_MTH +1];
	double DebugVector[DEBUG_NUM_COLS_LIE_RS_DE][SUBACCT_MAX_PERIOD_MTH +1];
	...
	Projection_BL_LIE_DE_GMWBL_01( const vector<vector <int>>& i_s_a,const vector<vector <int>>& f_s_a,
	...
	virtual void Write_Debug_File()
	{
		...
		this->debug_file << "BASIS_KAPITAL_LIE_RS_DE,GMDB_LIE_RS_DE,COUPON_VALUE_LIE_RS_DE,ADD_COUPON_RATCHET_LIE_RS_DE,ADD_COUPON_BONUS_LIE_RS_DE,FUND_FEES_VAL_LIE_RS_DE,DYNAMIC_LAPSE_LIE_RS_DE,ITM_VAL_LIE_RS_DE,LAPSE_VAL_LIE_RS_DE,HEDGE_COST_FEES_LIE_RS_DE,FIXED_CHARGE_VALUE_LIE_RS_DE,";
		this->debug_file << endl;

		for ( int t=0; t<= this->ModelPoint->Calc_Remaining_Months; t++)
		{
			this->debug_file <<setprecision(15)<< t << ",";
			for(int col = 0; col < TOTAL_CONSTVECTOR_COLS_LIE_RS_DE ; col++)
			...
			for(int col = 0; col < DEBUG_NUM_COLS_LIE_RS_DE ; col++)
...
//BG: NAMES CHANGED, FORMULAS NOT. HAS STILL TO BE CHECKED AND ADJUSTED! 
	...
	void Calculate_ConstVector_timing()
	{
		...
		for( int t=1; t <= this->ModelPoint->Calc_Remaining_Months ; t++){
			...
			if(t==1){
				ConstVector[DURATION_MONTH_LIE_RS_DE][0]=(int)((this->ModelPoint->Calc_Duration_Months+11)%12);
				ConstVector[DURATION_MONTH_LIE_RS_DE][1]=(int)(this->ModelPoint->Calc_Duration_Months%12);
				ConstVector[DURATION_YEAR_LIE_RS_DE][1]=(int)(this->ModelPoint->Calc_Duration_Months/12.);
				ConstVector[DURATION_YEAR_LIE_RS_DE][0]=max((int)((this->ModelPoint->Calc_Duration_Months-1)/12.),0);
				age_month_local=this->ModelPoint->Calc_Age_Month_1;
				ConstVector[AGE_LIE_RS_DE][1]=this->ModelPoint->Calc_Age_1;
				ConstVector[AGE_LIE_RS_DE][0]=(int)ConstVector[AGE_LIE_RS_DE][0]-(age_month_local==0);
				ConstVector[MONTH_LIE_RS_DE][0]=0;
				ConstVector[MONTH_LIE_RS_DE][1]=1;
				ConstVector[YEAR_LIE_RS_DE][1]=1;
				ConstVector[DATE_YEAR_LIE_RS_DE][1]=this->ModelPoint->Calc_Valn_Year;
				ConstVector[DATE_MONTH_LIE_RS_DE][1]=this->ModelPoint->Calc_Valn_Month;	
			}
			else
			{
				ConstVector[DURATION_MONTH_LIE_RS_DE][t]=(int)((ConstVector[DURATION_MONTH_LIE_RS_DE][t-1]!=11)*(ConstVector[DURATION_MONTH_LIE_RS_DE][t-1]+1));
				ConstVector[DURATION_YEAR_LIE_RS_DE][t]=(int)ConstVector[DURATION_YEAR_LIE_RS_DE][t-1];
				if((int)ConstVector[DURATION_MONTH_LIE_RS_DE][t]==0)
					ConstVector[DURATION_YEAR_LIE_RS_DE][t]+=1;
				age_month_local=(age_month_local!=11)*(age_month_local+1);
				ConstVector[AGE_LIE_RS_DE][t]=(int)ConstVector[AGE_LIE_RS_DE][t-1]+(age_month_local==0);
				ConstVector[MONTH_LIE_RS_DE][t]=(int)ConstVector[MONTH_LIE_RS_DE][t-1]+1;
				ConstVector[YEAR_LIE_RS_DE][t]=(int)(ConstVector[MONTH_LIE_RS_DE][t-1]/12)+1;			
				ConstVector[DATE_MONTH_LIE_RS_DE][t]=(int)((ConstVector[DATE_MONTH_LIE_RS_DE][t-1]!=12)*(ConstVector[DATE_MONTH_LIE_RS_DE][t-1]+1)+(ConstVector[DATE_MONTH_LIE_RS_DE][t-1]==12));
				ConstVector[DATE_YEAR_LIE_RS_DE][t]=ConstVector[DATE_YEAR_LIE_RS_DE][t-1];
				if((int)ConstVector[DATE_MONTH_LIE_RS_DE][t-1]==12)
					ConstVector[DATE_YEAR_LIE_RS_DE][t]+=1;
			}
		}
	}
	virtual void Calculate_ConstVector_guarantees()
	{
		for( int t=0; t <= this->ModelPoint->Calc_Remaining_Months ; t++)
		{
			ConstVector[IND_CONTRACT_PERIOD_LIE_RS_DE][t]=(int)(t<=this->ModelPoint->Calc_Remaining_Months);
			ConstVector[IND_DEFERAL_PERIOD_LIE_RS_DE][t]=(int)(t<=this->ModelPoint->Calc_Deferal_Duration_Months);
		...
		//BG: instead of model point bonus = "Y" here the new constant Bonus_Y_N from the assumption has been used (now integer instead of string)
			if(this->Assumption->Bonus_Y_N ==1){
				if(  ( (((t%12)+11)%12) == (((this->ModelPoint->Calc_Deferal_Duration_Months%12)+11)%12) ) &&  ( t > this->ModelPoint->Calc_Deferal_Duration_Months  )  )
					//ConstVector[IND_RATCHET_MONTH_LIE_RS_CHF][t]=1;
					  ConstVector[IND_BONUS_VALUATION_DATE_LIE_RS_DE][t]=1;	//BG: Formula changed, Need to be tested!
			}
			...
			if ((this->Assumption->Ratchet_Y_N==1)&&(t==this->ModelPoint->Calc_Deferal_Duration_Months)&&((int)ConstVector[IND_CONTRACT_PERIOD_LIE_RS_DE][t]==1) ) //BG: Check it, initial_def_per_dur<>0 needed?
				ConstVector[IND_RATCHET_MONTH_LIE_RS_DE][t]=1;
			else ConstVector[IND_RATCHET_MONTH_LIE_RS_DE][t]=0;

			ConstVector[IND_COUPON_DATE_LIE_RS_DE][t]=(int)((1-ConstVector[IND_DEFERAL_PERIOD_LIE_RS_DE][t]));
			if(this->ModelPoint->Calc_Freq_Pay==1){
				if(ConstVector[DURATION_MONTH_LIE_RS_DE][t]+1!=12)
					ConstVector[IND_COUPON_DATE_LIE_RS_DE][t]=0;		
			}
			else if(this->ModelPoint->Calc_Freq_Pay==2){
				if((ConstVector[DURATION_MONTH_LIE_RS_DE][t]+1!=12)&&(ConstVector[DURATION_MONTH_LIE_RS_DE][t]+1!=6))
					ConstVector[IND_COUPON_DATE_LIE_RS_DE][t]=0;			
			}
			else if(this->ModelPoint->Calc_Freq_Pay==3){
				if((ConstVector[DURATION_MONTH_LIE_RS_DE][t]+1!=12)&&(ConstVector[DURATION_MONTH_LIE_RS_DE][t]+1!=4)
					&&(ConstVector[DURATION_MONTH_LIE_RS_DE][t]+1!=8))
					ConstVector[IND_COUPON_DATE_LIE_RS_DE][t]=0;
			}
			else if(this->ModelPoint->Calc_Freq_Pay==4){
				if((ConstVector[DURATION_MONTH_LIE_RS_DE][t]+1!=12)&&(ConstVector[DURATION_MONTH_LIE_RS_DE][t]+1!=3)
					&&(ConstVector[DURATION_MONTH_LIE_RS_DE][t]+1!=6)&&(ConstVector[DURATION_MONTH_LIE_RS_DE][t]+1!=9))
					ConstVector[IND_COUPON_DATE_LIE_RS_DE][t]=0;
			}
			
			ConstVector[IND_DB_PERIOD_LIE_RS_DE][t]=ConstVector[IND_CONTRACT_PERIOD_LIE_RS_DE][t]*(this->Assumption->DB_Y_N ==1);
	...
	virtual void Calculate_ConstVector_decrement()
	{
		...
		ConstVector[DEATH_R_LIE_RS_DE][0]=0.;
		ConstVector[ALLE_VERBLEIBEND_LIE_RS_DE][0]=1.;
		...
		if(this->Assumption->Mortality_Y_N==1)
		{
			for( int t=1; t <= this->ModelPoint->Calc_Remaining_Months ; t++)
			{
				if(indic_1!=0)
					qx_month_1_local=this->Assumption->Get_mortality_rates_vector(this->ModelPoint->Gender_1_Bl,
											(int)ConstVector[DATE_YEAR_LIE_RS_DE][t],this->ModelPoint->Calc_Age_1,this->ModelPoint->Calc_Valn_Year);
				if(indic_2!=0)
					qx_month_2_local=this->Assumption->Get_mortality_rates_vector(this->ModelPoint->Gender_2_Bl,
											(int)ConstVector[DATE_YEAR_LIE_RS_DE][t],this->ModelPoint->Calc_Age_2,this->ModelPoint->Calc_Valn_Year);
				...
				ConstVector[ALLE_VERBLEIBEND_LIE_RS_DE][t]=local_surival_t+local_nur_1VP_t+local_nur_2VP_t;
				...
				if(ConstVector[ALLE_VERBLEIBEND_LIE_RS_DE][t-1]!=0.)
					ConstVector[DEATH_R_LIE_RS_DE][t]=local_Alle_Tote/ConstVector[ALLE_VERBLEIBEND_LIE_RS_DE][t-1];
				else
					ConstVector[DEATH_R_LIE_RS_DE][t]=1.;
				...
				if(this->Assumption->Longevity_Y_N==1)
				{
					...
					if(alle_verbleibend_t_bef_longevity!=0.)
						ConstVector[DEATH_R_LIE_RS_DE][t]=local_Alle_Tote_longevity/alle_verbleibend_t_bef_longevity;
					else
						ConstVector[DEATH_R_LIE_RS_DE][t]=1.;
			...
			for( int t=1; t <= this->ModelPoint->Calc_Remaining_Months ; t++)
				ConstVector[ALLE_VERBLEIBEND_LIE_RS_DE][t]=1.;
		}
	}
	void InitProjection()
	{
			...
			if(t==0){
				if(this->Assumption->Mortality_Lapse_Y_N==1){
					for( int i=0; i <= this->ModelPoint->Calc_Remaining_Months ; i++)
						sumprod_dynamic_lapse+=MainProjArrayVector[DISCOUNT_FACTOR_BL][i]*ConstVector[ALLE_VERBLEIBEND_LIE_RS_DE][i]*ConstVector[IND_COUPON_DATE_LIE_RS_DE][i];
					if(ConstVector[ALLE_VERBLEIBEND_LIE_RS_DE][t]==0.)
						sumprod_dynamic_lapse=0.;
					else
						sumprod_dynamic_lapse/=ConstVector[ALLE_VERBLEIBEND_LIE_RS_DE][t];
				}
				else{
					for( int i=0; i <= this->ModelPoint->Calc_Remaining_Months ; i++)
						sumprod_dynamic_lapse+=MainProjArrayVector[DISCOUNT_FACTOR_BL][i]*ConstVector[IND_COUPON_DATE_LIE_RS_DE][i];
				}
			...
			else
			{
				...
				if(this->Assumption->Mortality_Lapse_Y_N==1){
					if(ConstVector[ALLE_VERBLEIBEND_LIE_RS_DE][t]==0.)
						...
						sumprod_dynamic_lapse=(sumprod_dynamic_lapse*ConstVector[ALLE_VERBLEIBEND_LIE_RS_DE][t-1]-
							MainProjArrayVector[DISCOUNT_FACTOR_BL][t-1]*ConstVector[ALLE_VERBLEIBEND_LIE_RS_DE][t-1]*ConstVector[IND_COUPON_DATE_LIE_RS_DE][t-1])/
							ConstVector[ALLE_VERBLEIBEND_LIE_RS_DE][t];
				}
				else
					sumprod_dynamic_lapse=sumprod_dynamic_lapse-MainProjArrayVector[DISCOUNT_FACTOR_BL][t-1]*ConstVector[IND_COUPON_DATE_LIE_RS_DE][t-1];
				////////////////////End Dynamic lapse
				
				///////////////////Mortality rate
				mortality_rate=ConstVector[DEATH_R_LIE_RS_DE][t];
				///////////////////End Mortality rate
				...
				claims_wb_t=max(0.,annuity_per_freq*ConstVector[IND_COUPON_DATE_LIE_RS_DE][t]-account_value);	
				pvs[curr_shock_number][PV_CLAIMS_GMWB_BL]+=claims_wb_t*MainProjArrayVector[DISCOUNT_FACTOR_BL][t];	
				account_value-=min(account_value,((annuity_per_freq+add_coupon_from_ratchet*ConstVector[IND_RATCHET_MONTH_LIE_RS_DE][t-1]+add_coupon_from_bonus*ConstVector[IND_BONUS_VALUATION_DATE_LIE_RS_DE][t-1]))*ConstVector[IND_COUPON_DATE_LIE_RS_DE][t]);
				...
				claims_db_t=max(0.,gmdb*(1.-lapse)-account_value)*(1 == ConstVector[IND_DB_PERIOD_LIE_RS_DE][t])*mortality_rate;	
				...
				gmdb=max(0.,survival_index_by_step*gmdb)*(1 == ConstVector[IND_DB_PERIOD_LIE_RS_DE][t]);
				...
				basis_capital=survival_index_by_step*max(0.,basis_capital-annuity_per_freq*ConstVector[IND_COUPON_DATE_LIE_RS_DE][t-1]);
				basis_capital_1=basis_capital;
				if((int)ConstVector[IND_BONUS_VALUATION_DATE_LIE_RS_DE][t]==1)
				...
				add_coupon_from_ratchet=ConstVector[IND_RATCHET_MONTH_LIE_RS_DE][t]*(  max( account_value*this->ModelPoint->Conversion_Rate_Bl/this->ModelPoint->Calc_Freq_Pay/(1.+this->ModelPoint->Withrawals_Mode_Discount_Bl),annuity_per_freq ) - annuity_per_freq );
				add_coupon_from_bonus=ConstVector[IND_BONUS_VALUATION_DATE_LIE_RS_DE][t]*(  max( account_value*this->ModelPoint->Conversion_Rate_Bl/this->ModelPoint->Calc_Freq_Pay/(1.+this->ModelPoint->Withrawals_Mode_Discount_Bl),annuity_per_freq ) - annuity_per_freq );
				if((int)ConstVector[IND_RATCHET_MONTH_LIE_RS_DE][t]==1 || (int)ConstVector[IND_BONUS_VALUATION_DATE_LIE_RS_DE][t]==1 )
					basis_capital=max(account_value,(annuity_per_freq+add_coupon_from_ratchet+add_coupon_from_bonus)/(this->ModelPoint->Conversion_Rate_Bl/this->ModelPoint->Calc_Freq_Pay/(1.+this->ModelPoint->Withrawals_Mode_Discount_Bl)));
				///////////////////End Basis Capital + coupon calculus				
			}
			if(GenerateDebugFiles) //If debuging the porjection : not used in production
			{
				this->set_value_on_debug_file(this->account_value,ACCOUNT_VALUE_END_LIE_RS_DE, t);
				this->set_value_on_debug_file(this->account_value_2,ACCOUNT_VALUE_BEF_CHARGES_LIE_RS_DE, t);
				this->set_value_on_debug_file(this->survival_index_by_step,SURVIVAL_INDEX_STEP_LIE_RS_DE, t);
				this->set_value_on_debug_file(this->survival_index,SURVIVAL_INDEX_LIE_RS_DE, t);
				this->set_value_on_debug_file(this->claims_wb_t,CLAIMS_WB_LIE_RS_DE, t);
				this->set_value_on_debug_file(this->claims_db_t,CLAIMS_DB_LIE_RS_DE, t);
				this->set_value_on_debug_file(this->basis_capital,BASIS_KAPITAL_LIE_RS_DE, t);
				this->set_value_on_debug_file(this->gmdb,GMDB_LIE_RS_DE, t);
				this->set_value_on_debug_file(this->annuity_per_freq,COUPON_VALUE_LIE_RS_DE, t);
				this->set_value_on_debug_file(this->add_coupon_from_ratchet,ADD_COUPON_RATCHET_LIE_RS_DE, t);
				this->set_value_on_debug_file(this->add_coupon_from_bonus,ADD_COUPON_BONUS_LIE_RS_DE, t);
				this->set_value_on_debug_file(this->fund_fees,FUND_FEES_VAL_LIE_RS_DE, t);
				this->set_value_on_debug_file(this->lapse,DYNAMIC_LAPSE_LIE_RS_DE, t);
				this->set_value_on_debug_file(this->dynamic_lapse_ratio,ITM_VAL_LIE_RS_DE, t);
				this->set_value_on_debug_file(this->lapse_val,LAPSE_VAL_LIE_RS_DE, t);
				this->set_value_on_debug_file(this->hedge_cost_fees,HEDGE_COST_FEES_LIE_RS_DE, t);
				this->set_value_on_debug_file(this->fixed_charges,FIXED_CHARGE_VALUE_LIE_RS_DE, t);
	...
	void PresentValueCalculation(double **pvs,int curr_shock_number,bool delta)
	{
		...
		pvs[curr_shock_number][PV_EHC_NEW_RE_RATE_BL]=this->ModelPoint->Info_Bl;
