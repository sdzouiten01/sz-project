...
class Assumption_BL_LIE_CH_GMWLB_01		//GMWLB instead GMWB
{
	public:
		// Assumptions tables------------------------------------------------
		...
		int Ratchet_Y_N;
		int Perf_Bonus_Y_N;
		double Percentage_Perf;  //BG: maybe not needed anymore
		double Bonus_rate;
		double Perf_Level;	//BG: new
		int Use_Lapse_Y_N;
		double Base_Lapse;
...
public:
	...
	Assumption_BL_LIE_CH_GMWB_01(const char * tables)
	{
		...
		Perf_Level = atof(this->assump->search("Perf_Level").c_str());				//BG: new
		Bonus_rate=atof(this->assump->search("Bonus_rate").c_str());
		Percentage_Perf=atof(this->assump->search("Percentage_Perf").c_str());		//BG: maybe not needed anymore
		...
		Lapse_Charge=atof(this->assump->search("Lapse_Charge").c_str());
		...
		Max_Age=atof(this->assump->search("Max_age").c_str());
...
public:
	...
	{
		this->ModelPoint = dynamic_cast<Model_Point_BL_RS_CH_LIE_01*>(mp);
	...
	virtual void initialise_mp_calculation()
	{
		...
		//this->ModelPoint->Calc_Remaining_Months=min(720,this->Assumption->Max_Age+1-this->ModelPoint->Age_at_valn1;
		this->ModelPoint->Calc_Duration_Bl=this->ModelPoint->Calc_Remaining_Months+this->ModelPoint->Calc_Duration_Months;
		...
		if(this->ModelPoint->Freq_Mode_Bl=="MON")
			this->ModelPoint->Calc_Freq_Pay=12;
		else if(this->ModelPoint->Freq_Mode_Bl=="QUA")
			this->ModelPoint->Calc_Freq_Pay=4;
		else if(this->ModelPoint->Freq_Mode_Bl=="SEM")
			this->ModelPoint->Calc_Freq_Pay=2;
		else
			this->ModelPoint->Calc_Freq_Pay=1;

		//this->ModelPoint->Calc_Coupon_by_Freq=this->ModelPoint->Guarantee_Bl/this->ModelPoint->Calc_Freq_Pay;		Guarantee_BL replaced by Curr_Gmwb_Bl
