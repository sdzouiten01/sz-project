...
#include "head_BL_LIE_RST_DEIT.h"
#include "head_BL.h"			//BG: needed? yes! but why not possible?
using namespace std;

extern ofstream trace;
class Assumption_BL_LIE_DEIT_GMWB_01			//BG: changed, definition somewhere else needed?
{
	public:
		// Assumptions tables------------------------------------------------
...
public:
	...
	Assumption_BL_LIE_DEIT_GMWB_01(const char * tables)
	{
		...
		Base_Static_Lapse=atof(this->assump->search("Base_Lapse").c_str());
		...
		Guarantee_Charges=atof(this->assump->search("RRC").c_str());
		Fixed_Fees=atof(this->assump->search("Fixed_fee").c_str());;
	...
	{
		this->index1_l_local=(sex == "M")*(current_year-2007)+(sex != "M")*(current_year-2007+88);
...
public:
	...
	double ConstVector[TOTAL_CONSTVECTOR_COLS_LIE_RST_DE][SUBACCT_MAX_PERIOD_MTH +1];
	double MainProjArrayVector[MAIN_PROJ_NUM_COLS_BL][SUBACCT_MAX_PERIOD_MTH +1];
	double DebugVector[DEBUG_NUM_COLS_LIE_RST_DE][SUBACCT_MAX_PERIOD_MTH +1];
	...
	Projection_BL_LIE_DEIT_GMWB_01( const vector<vector <int>>& i_s_a,const vector<vector <int>>& f_s_a,
			...
			int t_high_shorter, bool l_longevity_tables):Projection(t_high_shorter)
	{
		...
		this->Assumption =new Assumption_BL_LIE_DEIT_GMWB_01(tables);
	...
	virtual void Write_Debug_File()
	{
		...
		*/
		this->debug_file << "TIME,DURATION_MONTH_LIE_DE,DURATION_YEAR_LIE_DE,AGE_LIE_DE,IND_CONTRACT_PERIOD_LIE_DE,IND_DEFERAL_PERIOD_LIE_DE,IND_RATCHET_MONTH_LIE_DE,IND_COUPON_DATE_LIE_DE,IND_BONUS_VALUATION_DATE_LIE_DE,IND_DB_PERIOD_LIE_DE,ALLE_VERBLEIBEND_LIE_DE,DEATH_R_LIE_DE,YEAR_LIE_DE,MONTH_LIE_DE,";
		this->debug_file << "DATE_YEAR_LIE_DE,DATE_MONTH_LIE_DE,FUND_RETURN_BL,DISCOUNT_FACTOR_BL,ACCOUNT_VALUE_END_LIE_DE,ACCOUNT_VALUE_BEF_CHARGES_LIE_DE,SURVIVAL_INDEX_STEP_LIE_DE,SURVIVAL_INDEX_LIE_DE,CLAIMS_WB_LIE_DE,CLAIMS_DB_LIE_DE,BASIS_KAPITAL_LIE_DE,GMDB_LIE_DE,";
		this->debug_file << "COUPON_VALUE_LIE_DE,ADD_COUPON_RATCHET_LIE_DE,PERFORMANCE_BONUS_VAL_LIE_DE,BONUS_VAL_LIE_DE,FUND_FEES_VAL_LIE_DE,DYNAMIC_LAPSE_LIE,ITM_VAL_LIE_DE,LAPSE_VAL_LIE_DE,MORT_VAL_LIE_DE,HEDGE_COST_FEES_LIE_DE,FIXED_CHARGE_VALUE_LIE_DE,";
		this->debug_file << endl;

		for ( int t=0; t<= this->ModelPoint->Calc_Remaining_Months; t++)
		{
			this->debug_file <<setprecision(15)<< t << ",";
			for(int col = 0; col < TOTAL_CONSTVECTOR_COLS_LIE_RST_DE ; col++)
			...
			for(int col = 0; col < DEBUG_NUM_COLS_LIE_RST_DE ; col++)
	...
	{
		this->ModelPoint = dynamic_cast<Model_Point_BL_RST_DEIT_LIE_01*>(mp);		//BG changed, correct?
...
//BG: NAMES CHANGED, FORMULAS NOT. HAS STILL TO BE CHECKED AND ADJUSTED! 
		...
		this->ModelPoint->Calc_fixed_fees_p_m=this->Assumption->Fixed_Fees/12.;
		
		if(this->ModelPoint->Withdrawals_Mode_Bl=="MON")
			this->ModelPoint->Calc_Freq_Pay=12;
		else if(this->ModelPoint->Withdrawals_Mode_Bl=="QUA")
			this->ModelPoint->Calc_Freq_Pay=4;
		else if(this->ModelPoint->Withdrawals_Mode_Bl=="SEM")
			this->ModelPoint->Calc_Freq_Pay=2;
		else
			this->ModelPoint->Calc_Freq_Pay=1;

		if(this->ModelPoint->Prod_Code_Bl=="LIE_IT_WBD01")
			this->ModelPoint->Calc_GMDB_Y_N=1;
		else
			this->ModelPoint->Calc_GMDB_Y_N=0;
		...
		this->ModelPoint->Calc_WB_Initial=this->ModelPoint->Calc_Coupon_by_Freq*this->ModelPoint->Calc_NB_Coupons;		
		...
		if(this->ModelPoint->Calc_Deferal_Duration_Months<=0)
			...
			this->ModelPoint->Calc_BK0=this->ModelPoint->Calc_WB_Initial;
		//this->ModelPoint->Calc_BK0=this->ModelPoint->Base_Capital_Bl;

		this->ModelPoint->Calc_RRC_p_m=this->Assumption->Guarantee_Charges/12.;
		...
		this->ModelPoint->Calc_Gaur_Charge_Rate=this->Assumption->Guarantee_Charges;
	...
	void Calculate_ConstVector_timing()
	{
		...
		for( int t=1; t <= this->ModelPoint->Calc_Remaining_Months ; t++){
			...
			if(t==1){
				ConstVector[DURATION_MONTH_LIE_RST_DE][0]=(int)((this->ModelPoint->Calc_Duration_Months+11)%12);
				ConstVector[DURATION_MONTH_LIE_RST_DE][1]=(int)(this->ModelPoint->Calc_Duration_Months%12);
				ConstVector[DURATION_YEAR_LIE_RST_DE][1]=(int)(this->ModelPoint->Calc_Duration_Months/12.);
				ConstVector[DURATION_YEAR_LIE_RST_DE][0]=max((int)((this->ModelPoint->Calc_Duration_Months-1)/12.),0);
				age_month_local=this->ModelPoint->Calc_Age_Month_1;
				ConstVector[AGE_LIE_RST_DE][1]=this->ModelPoint->Calc_Age_1;
				ConstVector[AGE_LIE_RST_DE][0]=(int)ConstVector[AGE_LIE_RST_DE][0]-(age_month_local==0);
				ConstVector[MONTH_LIE_RST_DE][0]=0;
				ConstVector[MONTH_LIE_RST_DE][1]=1;
				ConstVector[YEAR_LIE_RST_DE][1]=1;
				ConstVector[DATE_YEAR_LIE_RST_DE][1]=this->ModelPoint->Calc_Valn_Year;
				ConstVector[DATE_MONTH_LIE_RST_DE][1]=this->ModelPoint->Calc_Valn_Month;	
			}
			else
			{
				ConstVector[DURATION_MONTH_LIE_RST_DE][t]=(int)((ConstVector[DURATION_MONTH_LIE_RST_DE][t-1]!=11)*(ConstVector[DURATION_MONTH_LIE_RST_DE][t-1]+1));
				ConstVector[DURATION_YEAR_LIE_RST_DE][t]=(int)ConstVector[DURATION_YEAR_LIE_RST_DE][t-1];
				if((int)ConstVector[DURATION_MONTH_LIE_RST_DE][t]==0)
					ConstVector[DURATION_YEAR_LIE_RST_DE][t]+=1;
				age_month_local=(age_month_local!=11)*(age_month_local+1);
				ConstVector[AGE_LIE_RST_DE][t]=(int)ConstVector[AGE_LIE_RST_DE][t-1]+(age_month_local==0);
				ConstVector[MONTH_LIE_RST_DE][t]=(int)ConstVector[MONTH_LIE_RST_DE][t-1]+1;
				ConstVector[YEAR_LIE_RST_DE][t]=(int)(ConstVector[MONTH_LIE_RST_DE][t-1]/12)+1;			
				ConstVector[DATE_MONTH_LIE_RST_DE][t]=(int)((ConstVector[DATE_MONTH_LIE_RST_DE][t-1]!=12)*(ConstVector[DATE_MONTH_LIE_RST_DE][t-1]+1)+(ConstVector[DATE_MONTH_LIE_RST_DE][t-1]==12));
				ConstVector[DATE_YEAR_LIE_RST_DE][t]=ConstVector[DATE_YEAR_LIE_RST_DE][t-1];
				if((int)ConstVector[DATE_MONTH_LIE_RST_DE][t-1]==12)
					ConstVector[DATE_YEAR_LIE_RST_DE][t]+=1;
			}
		}
	}
	virtual void Calculate_ConstVector_guarantees()
	{
		for( int t=0; t <= this->ModelPoint->Calc_Remaining_Months ; t++)
		{
			ConstVector[IND_CONTRACT_PERIOD_LIE_RST_DE][t]=(int)(t<=this->ModelPoint->Calc_Remaining_Months);
			ConstVector[IND_DEFERAL_PERIOD_LIE_RST_DE][t]=(int)(t<=this->ModelPoint->Calc_Deferal_Duration_Months);
			...
			if(this->Assumption->Bonus_Y_N ==1){
				if(  ( (((t%12)+11)%12) == (((this->ModelPoint->Calc_Deferal_Duration_Months%12)+11)%12) ) &&  ( t > this->ModelPoint->Calc_Deferal_Duration_Months  )  )
					  ConstVector[IND_BONUS_VALUATION_DATE_LIE_RST_DE][t]=1;
			}
			if ((this->Assumption->Ratchet_Y_N==1)&&(t==this->ModelPoint->Calc_Deferal_Duration_Months)&&((int)ConstVector[IND_CONTRACT_PERIOD_LIE_RST_DE][t]==1) )
				ConstVector[IND_RATCHET_MONTH_LIE_RST_DE][t]=1;
			else ConstVector[IND_RATCHET_MONTH_LIE_RST_DE][t]=0;

			ConstVector[IND_COUPON_DATE_LIE_RST_DE][t]=(int)((1-ConstVector[IND_DEFERAL_PERIOD_LIE_RST_DE][t]));
			if(this->ModelPoint->Calc_Freq_Pay==1){
				if(ConstVector[DURATION_MONTH_LIE_RST_DE][t]+1!=12)
					ConstVector[IND_COUPON_DATE_LIE_RST_DE][t]=0;		
			}
			else if(this->ModelPoint->Calc_Freq_Pay==2){
				if((ConstVector[DURATION_MONTH_LIE_RST_DE][t]+1!=12)&&(ConstVector[DURATION_MONTH_LIE_RST_DE][t]+1!=6))
					ConstVector[IND_COUPON_DATE_LIE_RST_DE][t]=0;			
			}
			else if(this->ModelPoint->Calc_Freq_Pay==3){
				if((ConstVector[DURATION_MONTH_LIE_RST_DE][t]+1!=12)&&(ConstVector[DURATION_MONTH_LIE_RST_DE][t]+1!=4)
					&&(ConstVector[DURATION_MONTH_LIE_RST_DE][t]+1!=8))
					ConstVector[IND_COUPON_DATE_LIE_RST_DE][t]=0;
			}
			else if(this->ModelPoint->Calc_Freq_Pay==4){
				if((ConstVector[DURATION_MONTH_LIE_RST_DE][t]+1!=12)&&(ConstVector[DURATION_MONTH_LIE_RST_DE][t]+1!=3)
					&&(ConstVector[DURATION_MONTH_LIE_RST_DE][t]+1!=6)&&(ConstVector[DURATION_MONTH_LIE_RST_DE][t]+1!=9))
					ConstVector[IND_COUPON_DATE_LIE_RST_DE][t]=0;
			}

			//BG: Check the following formula again!!!
			ConstVector[IND_DB_PERIOD_LIE_RST_DE][t]=ConstVector[IND_CONTRACT_PERIOD_LIE_RST_DE][t]*(this->ModelPoint->Calc_GMDB_Y_N ==1)*(this->Assumption->Max_Age_Mortality > ConstVector[AGE_LIE_RST_DE][t])*(this->Assumption->Max_Age_DB > ConstVector[AGE_LIE_RST_DE][t]);
	...
	virtual void Calculate_ConstVector_decrement()
	{
		...
		ConstVector[DEATH_R_LIE_RST_DE][0]=0.;
		ConstVector[ALLE_VERBLEIBEND_LIE_RST_DE][0]=1.;
		...
		if(this->Assumption->Mortality_Y_N==1)
		{
			for( int t=1; t <= this->ModelPoint->Calc_Remaining_Months ; t++)
			{
				if(indic_1!=0)
					qx_month_1_local=this->Assumption->Get_mortality_rates_vector(this->ModelPoint->Gender_1_Bl,
											(int)ConstVector[DATE_YEAR_LIE_RST_DE][t],this->ModelPoint->Calc_Age_1,this->ModelPoint->Calc_Valn_Year);
				/*if(indic_2!=0)
					qx_month_2_local=this->Assumption->Get_mortality_rates_vector(this->ModelPoint->Gender_2_Bl,
											(int)ConstVector[DATE_YEAR_LIE_RST_DE][t],this->ModelPoint->Calc_Age_2,this->ModelPoint->Calc_Valn_Year);
				*/
				...
				ConstVector[ALLE_VERBLEIBEND_LIE_RST_DE][t]=local_surival_t;  //+local_nur_1VP_t+local_nur_2VP_t;
				local_Alle_Tote=local_surival_t_bef*qx_month_1_local;//*qx_month_2_local+local_nur_1VP_t_bef*qx_month_1_local+local_nur_2VP_t_bef*qx_month_2_local;
				
				if(ConstVector[ALLE_VERBLEIBEND_LIE_RST_DE][t-1]!=0.)
					ConstVector[DEATH_R_LIE_RST_DE][t]=local_Alle_Tote/ConstVector[ALLE_VERBLEIBEND_LIE_RST_DE][t-1];
				else
					ConstVector[DEATH_R_LIE_RST_DE][t]=1.;
				...
				if(this->Assumption->Longevity_Y_N==1)
				{
					...
					if(alle_verbleibend_t_bef_longevity!=0.)
						ConstVector[DEATH_R_LIE_RST_DE][t]=local_Alle_Tote_longevity/alle_verbleibend_t_bef_longevity;
					else
						ConstVector[DEATH_R_LIE_RST_DE][t]=1.;
			...
			for( int t=1; t <= this->ModelPoint->Calc_Remaining_Months ; t++)
				ConstVector[ALLE_VERBLEIBEND_LIE_RST_DE][t]=1.;
		}
	}
	void InitProjection()
	{
	...
	virtual void projection_base_contract(double **pvs,int curr_shock_number)
	{
		...
		for( int t=0; t <= this->ModelPoint->Calc_Remaining_Months ; t++)
		{
			if(t==0){
				if(this->Assumption->Mortality_Lapse_Y_N==1){
					for( int i=0; i <= this->ModelPoint->Calc_Remaining_Months ; i++)
						sumprod_dynamic_lapse+=MainProjArrayVector[DISCOUNT_FACTOR_BL][i]*ConstVector[ALLE_VERBLEIBEND_LIE_RST_DE][i]*ConstVector[IND_COUPON_DATE_LIE_RST_DE][i];
					if(ConstVector[ALLE_VERBLEIBEND_LIE_RST_DE][t]==0.)
						sumprod_dynamic_lapse=0.;
					else
						sumprod_dynamic_lapse/=ConstVector[ALLE_VERBLEIBEND_LIE_RST_DE][t];
				}
				else{
					for( int i=0; i <= this->ModelPoint->Calc_Remaining_Months ; i++)
						sumprod_dynamic_lapse+=MainProjArrayVector[DISCOUNT_FACTOR_BL][i]*ConstVector[IND_COUPON_DATE_LIE_RST_DE][i];
				}
			...
			else
			{
				...
				if(account_value==0.)
					dynamic_lapse_ratio=0.;
				else if(basis_capital==0.)
					dynamic_lapse_ratio=1000.;
				else
					dynamic_lapse_ratio=account_value/basis_capital;
				...
				if(this->Assumption->Mortality_Lapse_Y_N==1){
					if(ConstVector[ALLE_VERBLEIBEND_LIE_RST_DE][t]==0.)
						...
						sumprod_dynamic_lapse=(sumprod_dynamic_lapse*ConstVector[ALLE_VERBLEIBEND_LIE_RST_DE][t-1]-
							MainProjArrayVector[DISCOUNT_FACTOR_BL][t-1]*ConstVector[ALLE_VERBLEIBEND_LIE_RST_DE][t-1]*ConstVector[IND_COUPON_DATE_LIE_RST_DE][t-1])/
							ConstVector[ALLE_VERBLEIBEND_LIE_RST_DE][t];
				}
				else
					sumprod_dynamic_lapse=sumprod_dynamic_lapse-MainProjArrayVector[DISCOUNT_FACTOR_BL][t-1]*ConstVector[IND_COUPON_DATE_LIE_RST_DE][t-1];
				////////////////////End Dynamic lapse
				
				///////////////////Mortality rate
				mortality_rate=ConstVector[DEATH_R_LIE_RST_DE][t];
				///////////////////End Mortality rate
				...
				bonus_value=Performance_bonus_valuation*ConstVector[IND_BONUS_VALUATION_DATE_LIE_RST_DE][t-1];
				...
				claims_wb_t=max(0.,annuity_per_freq*ConstVector[IND_COUPON_DATE_LIE_RST_DE][t]+bonus_value-account_value);	
				...
				account_value-=min(account_value,((annuity_per_freq+add_coupon_from_ratchet*ConstVector[IND_RATCHET_MONTH_LIE_RST_DE][t-1]))*ConstVector[IND_COUPON_DATE_LIE_RST_DE][t]);
				//}
				//else 
				//	claims_wb_t=0.;
				...
				hedge_cost_fees=max(0.,account_value*this->ModelPoint->Calc_RRC_p_m)*(t != this->ModelPoint->Calc_Remaining_Months);
				...
				fixed_charges=min(account_value,this->ModelPoint->Calc_fixed_fees_p_m*account_value*survival_index)*(t != this->ModelPoint->Calc_Remaining_Months);
				...
				claims_db_t=max(0.,gmdb*(1.-lapse)-account_value)*(1 == ConstVector[IND_DB_PERIOD_LIE_RST_DE][t])*mortality_rate;	
				pvs[curr_shock_number][PV_CLAIMS_GMDB_BL]+=claims_db_t*MainProjArrayVector[DISCOUNT_FACTOR_BL][t];
				mort_val=account_value*mortality_rate;
				account_value*=(1.-mortality_rate);
				survival_index_by_step=(1.-lapse)*(1.-mortality_rate);
				survival_index*=survival_index_by_step;				
				gmdb=max(0.,survival_index_by_step*gmdb)*(1 == ConstVector[IND_DB_PERIOD_LIE_RST_DE][t]);
				...
				basis_capital=survival_index_by_step*max(0.,basis_capital-annuity_per_freq*ConstVector[IND_COUPON_DATE_LIE_RST_DE][t-1]);				
				basis_capital_1=basis_capital;
				if((int)ConstVector[IND_BONUS_VALUATION_DATE_LIE_RST_DE][t]==1)
				...
				//add_coupon_from_ratchet=ConstVector[IND_RATCHET_MONTH_LIE_RST_DE][t]*(  max( account_value*this->ModelPoint->Conversion_Rate_Bl/this->ModelPoint->Calc_Freq_Pay/(1.+this->ModelPoint->Withrawals_Mode_Discount_Bl),annuity_per_freq ) - annuity_per_freq );
				//add_coupon_from_ratchet=ConstVector[IND_RATCHET_MONTH_LIE_RST_DE][t]*(  max( account_value*this->ModelPoint->Conversion_Rate_Bl/this->ModelPoint->Calc_Freq_Pay,annuity_per_freq ) - annuity_per_freq );
				add_coupon_from_ratchet=ConstVector[IND_RATCHET_MONTH_LIE_RST_DE][t]*(  max( account_value/this->ModelPoint->Calc_NB_Coupons,annuity_per_freq ) - annuity_per_freq );
				if((int)ConstVector[IND_RATCHET_MONTH_LIE_RST_DE][t]==1)
					...
					basis_capital=max(account_value,(annuity_per_freq+add_coupon_from_ratchet)*this->ModelPoint->Calc_NB_Coupons);///(1.+this->ModelPoint->Withrawals_Mode_Discount_Bl)));
				///////////////////End Basis Capital + coupon calculus				

			}
			if(GenerateDebugFiles) //If debuging the porjection : not used in production
			{
				this->set_value_on_debug_file(this->account_value,ACCOUNT_VALUE_END_LIE_RST_DE, t);
				this->set_value_on_debug_file(this->account_value_2,ACCOUNT_VALUE_BEF_CHARGES_LIE_RST_DE, t);
				this->set_value_on_debug_file(this->survival_index_by_step,SURVIVAL_INDEX_STEP_LIE_RST_DE, t);
				this->set_value_on_debug_file(this->survival_index,SURVIVAL_INDEX_LIE_RST_DE, t);
				this->set_value_on_debug_file(this->claims_wb_t,CLAIMS_WB_LIE_RST_DE, t);
				this->set_value_on_debug_file(this->claims_db_t,CLAIMS_DB_LIE_RST_DE, t);
				this->set_value_on_debug_file(this->basis_capital_1,BASIS_KAPITAL_LIE_RST_DE, t);
				this->set_value_on_debug_file(this->gmdb,GMDB_LIE_RST_DE, t);
				this->set_value_on_debug_file(this->annuity_per_freq,COUPON_VALUE_LIE_RST_DE, t);
				this->set_value_on_debug_file(this->add_coupon_from_ratchet,ADD_COUPON_RATCHET_LIE_RST_DE, t);
				this->set_value_on_debug_file(this->Performance_bonus_valuation,PERFORMANCE_BONUS_VAL_LIE_RST_DE, t);	
				this->set_value_on_debug_file(this->bonus_value,BONUS_VAL_LIE_RST_DE, t);
				this->set_value_on_debug_file(this->fund_fees,FUND_FEES_VAL_LIE_RST_DE, t);
				this->set_value_on_debug_file(this->lapse,DYNAMIC_LAPSE_LIE_RST_DE, t);
				this->set_value_on_debug_file(this->dynamic_lapse_ratio,ITM_VAL_LIE_RST_DE, t);
				this->set_value_on_debug_file(this->lapse_val,LAPSE_VAL_LIE_RST_DE, t);
				this->set_value_on_debug_file(this->mort_val,MORT_VAL_LIE_RST_DE, t);
				this->set_value_on_debug_file(this->hedge_cost_fees,HEDGE_COST_FEES_LIE_RST_DE, t);
				this->set_value_on_debug_file(this->fixed_charges,FIXED_CHARGE_VALUE_LIE_RST_DE, t);
	...
	void PresentValueCalculation(double **pvs,int curr_shock_number,bool delta)
	{
		...
		pvs[curr_shock_number][BEGINING_ACCOUNT_VALUE_BL]=this->ModelPoint->Av_After_After_Bl*(this->ShockArray[this->ShockNumber][1]*0.01+1.);	
		// Calculate rates
		//pvs[curr_shock_number][PV_RRC_RATE_TOT_BL]=this->ModelPoint->Ong_Charge_1_Bl ;				
		pvs[curr_shock_number][PV_RRC_RATE_TOT_BL]=this->ModelPoint->Calc_Gaur_Charge_Rate;					
		pvs[curr_shock_number][PV_GUARANTEE_FEES_RATE_BL]=this->ModelPoint->Calc_Gaur_Charge_Rate;		
		pvs[curr_shock_number][PV_FIXED_FEES_RATE_BL]=this->ModelPoint->Calc_fixed_fees_p_m*12.;
		...
		pvs[curr_shock_number][PV_GUARANTEE_FEES_CHARGES_BL]=pvs[curr_shock_number][PV_RRC_TOT_CHARGES_BL]/(this->ModelPoint->Calc_RRC_p_m*12.)*this->ModelPoint->Calc_Gaur_Charge_Rate;
