...
#include "model_point_head_BL_RST_SWISS_01.h"
...
public:
	...
	Model_Point_BL_RST_SWISS_01(vector<string>& row, int rank, Product_configuration& product_conf,
	...
	Model_Point_BL_RST_SWISS_01(Model_Point_BL_RST_SWISS_01 * mp, int index_shock_size_in, int fund_shock_size_in):Model_Point()
	{
		this->get_timestamp();
	...
	{
		Prod_Code_Bl=row[PROD_CODE_BL];
		Valn_Date_Bl=row[VALN_DATE_BL];
		Fund_Fees_Bl= atof(row[FUND_FEES_BL].c_str());
		Ehc_Bl= atof(row[EHC_BL].c_str());
		New_Business_Bl=(atoi(row[NEW_BUSINESS_BL].c_str())==1);
		Pol_Id_Bl=row[POL_ID_BL];
		Prod_Name_Bl=row[PROD_NAME_BL];
		Init_Charge_Bl= atof(row[INIT_CHARGE_BL].c_str());
		Fund_Name_Bl=row[FUND_NAME_BL];
		Currency_Bl=row[CURRENCY_BL];
		Issue_Date_Bl=row[ISSUE_DATE_BL];
		Nb_Date_Bl=row[NB_DATE_BL];
		Inv_Date_Bl=row[INV_DATE_BL];
		Def_Period_Bl=atoi(row[DEF_PERIOD_BL].c_str());
		Duration_Bl=atoi(row[DURATION_BL].c_str());
		Annuity_Begin_Bl=row[ANNUITY_BEGIN_BL];
		Prod_Prem_Type_Bl=row[PROD_PREM_TYPE_BL];
		Premium_Bl= atof(row[PREMIUM_BL].c_str());
		Amount_Inv_Bl= atof(row[AMOUNT_INV_BL].c_str());
		Freq_Mode_Bl=row[FREQ_MODE_BL];
		Withdrawal_Rate_Bl= atof(row[WITHDRAWAL_RATE_BL].c_str());
		Inv_Protection_Bl= atof(row[INV_PROTECTION_BL].c_str());
		Benefit_Increase_Factor_Bl= atof(row[BENEFIT_INCREASE_FACTOR_BL].c_str());
		Withdrawal_Mode_discount_Bl= atof(row[WITHDRAWAL_MODE_DISCOUNT_BL].c_str());
		Guarantee_Bl= atof(row[GUARANTEE_BL].c_str());
		Accum_Withdrawal_Amount_Bl= atof(row[ACCUM_WITHDRAWAL_AMOUNT_BL].c_str());
		...
		Av_After_After_Bl= atof(row[AV_AFTER_AFTER_BL].c_str());
		Base_Capital_Bl= atof(row[BASE_CAPITAL_BL].c_str());
		Fund_Unit_Bef_Bef_Bl= atof(row[FUND_UNIT_BEF_BEF_BL].c_str());
		Fund_Unit_After_Bef_Bl= atof(row[FUND_UNIT_AFTER_BEF_BL].c_str());
		Fund_Unit_After_After_Bl= atof(row[FUND_UNIT_AFTER_AFTER_BL].c_str());
		AV0=Av_After_After_Bl;
		prod_code_key=Prod_Code_Bl;
		New_Re_Reinsurance_premium_Bl=atof(row[INFO].c_str());
		policy_id=Pol_Id_Bl;
		
		av_if_array[0] = atof(row[AV_AFTER_AFTER_BL].c_str());
		av_if_array[1] = 0.;
		av_if_array[2] = 0.;
		av_if_array[3] = 0.;
		av_if_array[4] = 0.;
