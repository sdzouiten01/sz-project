...
Annual_Swap_Rates_Shock_Up::Annual_Swap_Rates_Shock_Up(int swap_no,Rw_Scn* rw_scn,Assumptions* assumptions)
{
	...
	/*tst.open("Annual_Swap_Rates_Shock_Up.txt");
...
/*
double Annual_Swap_Rates_Shock_Up::interpolated_rate(double mty);
{
	double res=0;
	double product=1;
	for (int i=0;i<this->objects_needed.rw_scn->model_period;i++)
	{
		...
		res+=product*this->objects_needed.rw_scn.tab[this->objects_needed.rw_scn.model_period-1][i];
	}
}
*/
void Annual_Swap_Rates_Shock_Up::init_tab()
{
...
double Annual_Swap_Rates_Shock_Up::shock(int start_rate, int key_rate, int end_rate, int mty)
{
	if (start_rate==0)
	{
...
void Annual_Swap_Rates_Shock_Up::shock_up()
{
	for (int i =0;i<this->dim_x;++i)
	{
		for (int j =0;j<this->dim_y;++j)
...
Annual_Swap_Rates_Shock_Up::~Annual_Swap_Rates_Shock_Up(void)
{
	for(int i=0;i<this->dim_x;++i)
	{
		delete [] this->tab[i];
