...
class Assumption_BL_LUX_GMWBLD_01 
{
	public:
		// Assumptions tables------------------------------------------------
...
public:
	...
	Projection_BL_LUX_GMWBLD_01( const vector<vector <int>>& i_s_a,const vector<vector <int>>& f_s_a,
	...
	{
	   	char message[MAX_LENGTH_MESSAGE_EXCEPTION];
		sprintf_s(message, "Baloise LUX projection don't support scen_by_scen output");
		throw new GMXBException(message);	
	...
	virtual void initialise_mp_calculation()
	{
		...
		this->ModelPoint->Calc_WB_Rollup=this->ModelPoint->Benefit_Increase_Factor_Lux*(1. + this->ModelPoint->Rollup_Rate_Lux*this->ModelPoint->Calc_Initial_Deferal_Months/12.)*this->ModelPoint->Insur_Prem_Lux;
		
		this->ModelPoint->Calc_Initial_Coupon_by_Freq=this->ModelPoint->Calc_WB_Rollup*this->ModelPoint->Conversion_Rate_Lux/this->ModelPoint->Calc_Freq_Pay/(1.+this->ModelPoint->Withrawals_Mode_Discount_Lux);
			...
			else
			{
				...
				if((int)ConstVector[IND_RATCHET_MONTH_BL_LUX][t]==1)
				{
					if (this->ModelPoint->Calc_Initial_Deferal_Months==0)
						add_coupon_from_ratchet=this->ModelPoint->Benefit_Increase_Factor_Lux * max(0.,account_value-basis_capital)
								*this->ModelPoint->Conversion_Rate_Lux/this->ModelPoint->Calc_Freq_Pay/(1.+this->ModelPoint->Withrawals_Mode_Discount_Lux);
					else
