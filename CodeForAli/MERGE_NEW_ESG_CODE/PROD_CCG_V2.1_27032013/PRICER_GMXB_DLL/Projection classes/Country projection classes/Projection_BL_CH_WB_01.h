...
class Assumption_BL_CH_GMWB_01 
{
	public:
		// Assumptions tables------------------------------------------------
...
public:
	virtual ~Assumption_BL_CH_GMWB_01(){delete this->assump;}
	Assumption_BL_CH_GMWB_01(){}
	Assumption_BL_CH_GMWB_01(const char * tables)
	{
		// Assumptions tables------------------------------------------------
...
public:
	...
	//Assumption Class	
	Assumption_BL_CH_GMWB_01 *Assumption;

	//Model Point Class
	Model_Point_BL_RST_SWISS_01 *ModelPoint;
	
	//Tables used in the projection
	double ConstVector[TOTAL_CONSTVECTOR_COLS_BL_CH][SUBACCT_MAX_PERIOD_MTH +1];
	double MainProjArrayVector[MAIN_PROJ_NUM_COLS_BL][SUBACCT_MAX_PERIOD_MTH +1];
	double DebugVector[DEBUG_NUM_COLS_BL_CH][SUBACCT_MAX_PERIOD_MTH +1];
	...
	Projection_BL_CH_GMWB_01( const vector<vector <int>>& i_s_a,const vector<vector <int>>& f_s_a,
			...
			int t_high_shorter, bool l_longevity_tables):Projection(t_high_shorter)
	{
		...
		this->Assumption =new Assumption_BL_CH_GMWB_01(tables);
	...
	virtual void Write_Debug_File()
	{
		...
		this->debug_file << "TIME,DURATION_MONTH_BL_CH,IND_CONTRACT_PERIOD_BL_CH,IND_DEFERAL_PERIOD_BL_CH,IND_RATCHET_MONTH_BL_CH,IND_COUPON_DATE_BL_CH,IND_BONUS_VALUATION_DATE_BL_CH,IND_BONUS_PAYMENT_DATE_BL_CH,FUND_RETURN_BL,DISCOUNT_FACTOR_BL,YEAR_BL_CH,MONTH_BL_CH,DURATION_YEAR_BL_CH,";
		...
		for ( int t=0; t<= this->ModelPoint->Calc_Remaining_Months; t++)
		{
			...
			for(int col = 0; col < TOTAL_CONSTVECTOR_COLS_BL_CH ; col++)
			...
			for(int col = 0; col < DEBUG_NUM_COLS_BL_CH ; col++)
	...
	virtual void projection_base_contract(double **pvs,int curr_shock_number)
	{
		...
		for( int t=0; t <= this->ModelPoint->Calc_Remaining_Months ; t++)
		{
			if(t==0)
			{
				...
				claims_wb_t=0.;
				basis_capital=this->ModelPoint->Base_Capital_Bl;
				...
				pvs[curr_shock_number][PV_AV_TOT_BL]+=this->ModelPoint->Av_After_Bef_Bl;
	...
	void PresentValueCalculation(double **pvs,int curr_shock_number,bool delta)
	{
		...
		pvs[curr_shock_number][PV_LAPSE_CHARGES_BL]=0.;
