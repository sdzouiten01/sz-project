...
#include "model_point_head_BL_RS_CH_LIE_01.h"
#include "../../Projection classes/Country projection classes/head_BL_CH.h"			//BG: to change?
...
public:
	...
	double Ong_Charge_1_Bl;	//new, ong_acq_charge removed
	double Ong_Charge_2_Bl;	//new
	...
	string Inv_Date_Bl;	//NB_Date in BL_LIE_RS inforce
	int    Def_Period_Bl;
	//int    Duration_Bl;	//not in inforce, should be declared in calc!!! Is already declared!
	string Annuity_Begin_Date_Bl;	//annuity commencement date
	string Prem_Pay_Mode_Bl;
	double Insur_Prem_Bl;	//insurance_prem_after_tax in inforce
	double Amount_Invested_Bl;
	string Withdrawals_Mode_Bl;
	double Conversion_Rate_Bl;
	//double Rollup_Rate_Lux;
	double Def_Guarantie_Bl;	//check whether this is inv_protection now!?
	...
	//double Perf_Bonus_Level_Lux;	//now fixed in assumptions
	//int    Perf_Bonus_Type_Lux;	//now fixed in assumptions
	//double Add_Accum_Withrawal_Amount_Lux;
	double Perf_Bonus_Amount_Bl;
	double Av_Bef_Bef_Bl;		//new
	double Av_After_Bef_Bl;		//new
	double Av_After_After_Bl;   //new, replaced AV
	double Base_Capital_Bl;
	double Fund_Unit_Bef_Bef_Bl; //new
	double Fund_Unit_After_Bef_Bl; //new
	double Fund_Unit_After_After_Bl; //new, replaced Fund_units
	...
	Model_Point_BL_RS_CH_LIE_01(vector<string>& row, int rank, Product_configuration& product_conf,
	...
	Model_Point_BL_RS_CH_LIE_01(Model_Point_BL_RS_CH_LIE_01 * mp, int index_shock_size_in, int fund_shock_size_in):Model_Point()
	{
		this->get_timestamp();
	...
	{	
		Valn_Date_Bl=row[VALN_DATE_LIE_RS_CHF];	//BG: mapped to model_point_head_BL_RS_CH_LIE_01.h number?
		Fund_fees_Bl= atof(row[FUND_FEES_LIE_RS_CHF].c_str());
		Info_Bl=atof(row[INFO_LIE_RS_CHF].c_str());
		Prod_Code_Bl=row[PROD_CODE_LIE_RS_CHF];
		Ehc_Bl= atof(row[EHC_LIE_RS_CHF].c_str());
		New_Business_Bl=atoi(row[NEW_BUSINESS_LIE_RS_CHF].c_str());
		Pol_Id_Bl=row[POLICY_ID_LIE_RS_CHF];
		Person_Id_1_Bl=row[PERSON_ID_1_LIE_RS_CHF];
		Person_Id_2_Bl=row[PERSON_ID_2_LIE_RS_CHF];
		Init_Charge_Bl= atof(row[INIT_CHARGE_LIE_RS_CHF].c_str());
		...
		Account_Code_Bl=row[ACCOUNT_CODE_LIE_RS_CHF];
		Currency_Bl=row[CURRENCY_LIE_RS_CHF];
		...
		Issue_Date_Bl=row[ISSUE_DATE_LIE_RS_CHF];
		Inv_Date_Bl=row[INV_DATE_LIE_RS_CHF];
		Def_Period_Bl=atoi(row[DEF_PERIOD_LIE_RS_CHF].c_str());
		//Duration_Bl=atoi(row[DURATION_LIE_RS_CHF].c_str());
		Annuity_Begin_Date_Bl=row[ANNUITY_BEGIN_DATE_LIE_RS_CHF];
		Prem_Pay_Mode_Bl=row[PREMIUM_PAYMENT_MODE_LIE_RS_CHF];	
		Insur_Prem_Bl= atof(row[INSURANCE_PREMIUM_LIE_RS_CHF].c_str());
		Amount_Invested_Bl= atof(row[AMOUNT_INVESTED_LIE_RS_CHF].c_str());
		Withdrawals_Mode_Bl=row[WITHDRAWAL_MODE_LIE_RS_CHF];
		Conversion_Rate_Bl= atof(row[CONVERSION_RATE_LIE_RS_CHF].c_str());
		//Rollup_Rate_Bl= atof(row[ROLLUP_RATE_LIE_RS_CHF].c_str());
		Def_Guarantie_Bl= atof(row[DEF_GUAR_LIE_RS_CHF].c_str());
		//Inv_Protection_Bl= atof(row[INV_PROTECTION_LIE_RS_CHF].c_str());
		Benefit_Increase_Factor_Bl= atof(row[BENEFIT_INCREASE_FACTOR_LIE_RS_CHF].c_str());
		...
		Curr_Gmwb_Bl= atof(row[CURR_GMWB_LIE_RS_CHF].c_str());
		Accum_Withrawal_Amount_Bl= atof(row[ACCUM_WITHDRAWAL_AMOUNT_LIE_RS_CHF].c_str());
		//Perf_Bonus_Bl=row[PERF_BONUS_LIE_RS_CHF];
		//Perf_Bonus_Level_Bl= atof(row[PERF_BONUS_LEVEL_LIE_RS_CHF].c_str());
		//Perf_Bonus_Type_Bl=atoi(row[PERF_BONUS_TYPE_LIE_RS_CHF].c_str());
		//Add_Accum_Withrawal_Amount_Lux= atof(row[ADD_ACCUM_WITHDRAWAL_AMOUNT_LIE_RS_CHF].c_str());
		...
		Av_After_After_Bl= atof(row[ACC_VAL_AFT_AFT_LIE_RS_CHF].c_str());
		Base_Capital_Bl= atof(row[BASE_CAPITAL_LIE_RS_CHF].c_str());
		...
		Fund_Unit_After_After_Bl= atof(row[FUND_UNITS_AFT_AFT_LIE_RS_CHF].c_str());
		Unit_Price_Bl= atof(row[UNIT_PRICE_LIE_RS_CHF].c_str());
		//Reinsurance_Premium_Bl= atof(row[REINSURANCE_PREMIUM_LIE_RS_CHF].c_str());
		//Fund_Charge_Bl= atof(row[FUND_CHARGE_LIE_RS_CHF].c_str());
		//Upfront_Comm_Bl= atof(row[UPFRONT_COMM_LIE_RS_CHF].c_str());
		//Trail_Comm_Bl= atof(row[TRAIL_COMM_LIE_RS_CHF].c_str());
		AV0=Av_After_After_Bl;	
		prod_code_key=Prod_Code_Bl;
		policy_id=Pol_Id_Bl;		

		av_if_array[0] = atof(row[ACC_VAL_AFT_AFT_LIE_RS_CHF].c_str());
		av_if_array[1] = 0.;
		av_if_array[2] = 0.;
		av_if_array[3] = 0.;
		av_if_array[4] = 0.;
	...
	void HE_initialise_mp_from_inforce(Model_Point * m_p)
	{
		...
		//Tech_Issue_Age_Bl  = mp->Tech_Issue_Age_Bl;
		...
		//Rollup_Rate_Bl  = mp->Rollup_Rate_Bl;
		...
		//Gmdb_Rider_Bl  = mp->Gmdb_Rider_Bl;
		//Gmdb_Mode_Bl  = mp->Gmdb_Mode_Bl;
		//Gmdb_Max_Age_Bl  = mp->Gmdb_Max_Age_Bl;
		//Gmdb_Duration_Bl  = mp->Gmdb_Duration_Bl;
		//Gmdb_Bl  = mp->Gmdb_Bl;
		Curr_Gmwb_Bl  = mp->Curr_Gmwb_Bl;
		Accum_Withrawal_Amount_Bl  = mp->Accum_Withrawal_Amount_Bl;
		//Perf_Bonus_Bl  = mp->Perf_Bonus_Bl;	//BG now in assumption
		//Perf_Bonus_Level_Bl  = mp->Perf_Bonus_Level_Bl;
		//Perf_Bonus_Type_Bl  = mp->Perf_Bonus_Type_Bl;
		...
		//Reinsurance_Premium_Bl  = mp->Reinsurance_Premium_Bl;
		//Fund_Charge_Bl  = mp->Fund_Charge_Bl;
		//Upfront_Comm_Bl  = mp->Upfront_Comm_Bl;
		//Trail_Comm_Bl  = mp->Trail_Comm_Bl;
		AV0  = mp->AV0;
