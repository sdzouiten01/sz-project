#ifndef __Model_Point_BL_RST_DE_LIE_01_H_
#define __Model_Point_BL_RST_DE_LIE_01_H_

#include "../Model_Point.h"
#include "model_point_head_BL_RST_DEIT_LIE_01.h"
#include "../../Projection classes/Country projection classes/head_BL_CH.h"	

using namespace std;

class Model_Point_BL_RST_DEIT_LIE_01 : public Model_Point
{
public:
	...
	virtual ~Model_Point_BL_RST_DEIT_LIE_01(){}
	
	Model_Point_BL_RST_DEIT_LIE_01(vector<string>& row, int rank, Product_configuration& product_conf,
	...
	Model_Point_BL_RST_DEIT_LIE_01(Model_Point_BL_RST_DEIT_LIE_01 * mp, int index_shock_size_in, int fund_shock_size_in):Model_Point()
	{
		this->get_timestamp();
	...
	{	
		Valn_Date_Bl=row[VALN_DATE_LIE_RST_DE];
		Fund_fees_Bl= atof(row[FUND_FEES_LIE_RST_DE].c_str());
		Info_Bl=atof(row[INFO_LIE_RST_DE].c_str());
		Prod_Code_Bl=row[PROD_CODE_LIE_RST_DE];
		Ehc_Bl= atof(row[EHC_LIE_RST_DE].c_str());
		New_Business_Bl=atoi(row[NEW_BUSINESS_LIE_RST_DE].c_str());
		Pol_Id_Bl=row[POLICY_ID_LIE_RST_DE];
		Person_Id_1_Bl=row[PERSON_ID_1_LIE_RST_DE];
		//Person_Id_2_Bl=row[PERSON_ID_2_LIE_RS_CHF];
		//...PRODCODE MISSING?!
		Init_Charge_Bl= atof(row[INIT_CHARGE_LIE_RST_DE].c_str());
		...
		Account_Code_Bl=row[ACCOUNT_CODE_LIE_RST_DE];
		Currency_Bl=row[CURRENCY_LIE_RST_DE];
		...
		Issue_Date_Bl=row[ISSUE_DATE_LIE_RST_DE];
		//NB_Date??? needed for projection?
		Inv_Date_Bl=row[INV_DATE_LIE_RST_DE];
		Def_Period_Bl=atoi(row[DEF_PERIOD_LIE_RST_DE].c_str());
		Duration_Bl=atoi(row[DURATION_LIE_RST_DE].c_str());
		Annuity_Begin_Date_Bl=row[ANNUITY_BEGIN_DATE_LIE_RST_DE];
		Prem_Pay_Mode_Bl=row[PREMIUM_PAYMENT_MODE_LIE_RST_DE];	
		Insur_Prem_Bl= atof(row[INSURANCE_PREMIUM_LIE_RST_DE].c_str());
		Amount_Invested_Bl= atof(row[AMOUNT_INVESTED_LIE_RST_DE].c_str());
		Withdrawals_Mode_Bl=row[WITHDRAWAL_MODE_LIE_RST_DE];
		Conversion_Rate_Bl= atof(row[CONVERSION_RATE_LIE_RST_DE].c_str());
		//Rollup_Rate_Bl= atof(row[ROLLUP_RATE_LIE_RS_CHF].c_str());
		Def_Guarantie_Bl= atof(row[DEF_GUAR_LIE_RST_DE].c_str());
		//Inv_Protection_Bl= atof(row[INV_PROTECTION_LIE_RS_CHF].c_str());
		Benefit_Increase_Factor_Bl= atof(row[BENEFIT_INCREASE_FACTOR_LIE_RST_DE].c_str());
		...
		Curr_Gmwb_Bl= atof(row[CURR_GMWB_LIE_RST_DE].c_str());
		Accum_Withrawal_Amount_Bl= atof(row[ACCUM_WITHDRAWAL_AMOUNT_LIE_RST_DE].c_str());
		...
		Av_After_After_Bl= atof(row[ACC_VAL_AFT_AFT_LIE_RST_DE].c_str());
		Base_Capital_Bl= atof(row[BASE_CAPITAL_LIE_RST_DE].c_str());
		...
		Fund_Unit_After_After_Bl= atof(row[FUND_UNITS_AFT_AFT_LIE_RST_DE].c_str());
		Unit_Price_Bl= atof(row[UNIT_PRICE_LIE_RST_DE].c_str());
		...
		av_if_array[0] = atof(row[ACC_VAL_AFT_AFT_LIE_RST_DE].c_str());
		av_if_array[1] = 0.;
		av_if_array[2] = 0.;
		av_if_array[3] = 0.;
		av_if_array[4] = 0.;
