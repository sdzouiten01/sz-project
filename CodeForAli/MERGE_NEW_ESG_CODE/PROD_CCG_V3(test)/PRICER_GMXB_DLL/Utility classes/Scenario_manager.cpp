...
int scenario_manager::get_nb_scenarios(){return this->number_of_scenarios;}
double** scenario_manager::get_monthly_rates(int it_number){	return this->monthly_rates[it_number-1];}
...
void scenario_manager::init_ann_from_monthly_table(const string& file_name, int reset_value)
{
	...
	if (file_op)
	{
		...
		while(iteration < this->number_of_scenarios && !file_op.eof())
		{
			...
			if (row.length() > 0)
			{
				...
				retPtr[row.length()] = '\0';
				//int col = 0;
				iteration = atoi(strtok_s(retPtr, ",",&next_token)) - 1;
				period = atoi(strtok_s(NULL, ",",&next_token));
				// Skip the period 0
				if (period == 0)
				{
					delete[] retPtr;
				...
				char * returns;
				int nb_returns = 0;
				while ((returns = strtok_s(NULL, ",",&next_token)) != NULL)
				{	
	...
	else 
	{
		...
		sprintf_s(message, "The scenario file %s is not found", file_name.c_str());
...
*/
//! Yuan 
scenario_manager::scenario_manager(double ***va_scen,int number_of_scen,vector<int> scen_fund_col_vector, int yield_column_for_pvs_index,
...
*/
void scenario_manager::set_columns(vector<int>& scen_fund_col_vector, int yield_column_for_pvs_index, int lapse_dyn_treas_col_index, int rates_indic_index)
{
...
{
	UNREFERENCED_PARAMETER(swap_table);
	UNREFERENCED_PARAMETER(nb_date);
	this->reset_ann_from_monthly_table(reset_value);
	string row;
	string firstline;
	ifstream file_op(file_name.c_str() , ios::in);
...
void scenario_manager::display_ann_from_mthly(char * file_name)
{
	...
	sprintf_s(name, "C:\\%s.txt", file_name);
	ofstream log_ann;
	log_ann.open(name);
...
int scenario_manager :: init_t_high_shorter(const string& file_name)
{
	...
	if (file_op)
	{
		...
		{
			char message[MAX_LENGTH_MESSAGE_EXCEPTION];
			sprintf_s(message, "Error in function in scenario_manager :: init_t_high_shorter, (file_mode) period_temp%12!=0 || period_temp > 720" );
			throw new GMXBException(message);
		}
		else
		{
