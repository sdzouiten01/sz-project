#include "Projection_BL_LIE_DEIT_WBD_01.h"





Assumption_BL_LIE_DEIT_GMWB_01::~Assumption_BL_LIE_DEIT_GMWB_01(){}
Assumption_BL_LIE_DEIT_GMWB_01::Assumption_BL_LIE_DEIT_GMWB_01():Assumption_BL_LIE_CH_GMWBL_01(){}
Assumption_BL_LIE_DEIT_GMWB_01::Assumption_BL_LIE_DEIT_GMWB_01(const char * tables):Assumption_BL_LIE_CH_GMWBL_01(tables){ Max_Age_DB= atoi(this->assump->search("Max_Age_DB").c_str());}




Projection_BL_LIE_DEIT_GMWB_01::~Projection_BL_LIE_DEIT_GMWB_01(){	delete this->Assumption;}
Projection_BL_LIE_DEIT_GMWB_01::Projection_BL_LIE_DEIT_GMWB_01( const vector<vector <int>>& i_s_a,const char * tables, bool generate_dbg_files,const string& debug_file_path):Projection(i_s_a,tables,generate_dbg_files,debug_file_path)
	{
	...
	///////////////////////////////////////////////////////////////////////////////////
	//////////////////////////////////////////////////////////////////////////////////
void Projection_BL_LIE_DEIT_GMWB_01::set_Local_Parameters_to_zero()
	{
...
void Projection_BL_LIE_DEIT_GMWB_01::mySet_Debug_File_Header(){
		...
		this->debug_file << "DATE_YEAR_LIE_DE,DATE_MONTH_LIE_DE,FUND_RETURN,DISCOUNT_FACTOR,ACCOUNT_VALUE_END_LIE_DE,ACCOUNT_VALUE_BEF_CHARGES_LIE_DE,SURVIVAL_INDEX_STEP_LIE_DE,SURVIVAL_INDEX_LIE_DE,CLAIMS_WB_LIE_DE,CLAIMS_DB_LIE_DE,BASIS_KAPITAL_LIE_DE,GMDB_LIE_DE,";
...
void Projection_BL_LIE_DEIT_GMWB_01::set_current_model_point(Model_Point *mp)
	{
...
void Projection_BL_LIE_DEIT_GMWB_01::init_Count_scen_to_zero(){this->ModelPoint->Calc_Count_Scen=0;}
		int Projection_BL_LIE_DEIT_GMWB_01::Count_scen(){return this->ModelPoint->Calc_Count_Scen;}
void Projection_BL_LIE_DEIT_GMWB_01::Increase_Count_scen(){this->ModelPoint->Calc_Count_Scen +=1;}
	///////////////////////////////////////////////////////////////////////////////////
	//////////////////////////////////////////////////////////////////////////////////
void Projection_BL_LIE_DEIT_GMWB_01::initialise_mp_calculation()
	{
...
//BG: NAMES CHANGED, FORMULAS NOT. HAS STILL TO BE CHECKED AND ADJUSTED! 
		...
		this->ModelPoint->Calc_fixed_fees_p_m=this->Assumption->Fixed_Charge/12.;
		...
		if(this->ModelPoint->Prod_Code=="LIE_IT_WBD01")
		...
		this->ModelPoint->Calc_RRC_p_m=this->Assumption->Guarantee_Charge/12.;
		this->ModelPoint->Calc_Conversion_Rate_p_m=this->ModelPoint->Withdrawal_Rate/this->ModelPoint->Calc_Freq_Pay;
		this->ModelPoint->Calc_Max_Bonus_Amount=this->Assumption->Bonus_rate*this->ModelPoint->Premium;
		this->ModelPoint->Calc_Perf_Bonus_Amount_Bl=min(max(this->ModelPoint->Av_After_Bef_Bl-this->ModelPoint->Base_Capital,0.)*this->Assumption->Percentage_Perf,this->ModelPoint->Calc_Max_Bonus_Amount)*
		...
		this->ModelPoint->Calc_Gaur_Charge_Rate=this->Assumption->Guarantee_Charge;
		
		
	}
void Projection_BL_LIE_DEIT_GMWB_01::Calculate_ConstVector_timing()
	{
...
void Projection_BL_LIE_DEIT_GMWB_01::Calculate_ConstVector_guarantees()
	{
		for( int t=0; t <= this->ModelPoint->Calc_Remaining_Months ; t++)
		{
			...
			if(this->Assumption->Perf_Bonus_Y_N ==1){
				if(  ( (((t%12)+11)%12) == (((this->ModelPoint->Calc_Deferal_Duration_Months%12)+11)%12) ) &&  ( t > this->ModelPoint->Calc_Deferal_Duration_Months  )  )
					  ConstVector[IND_BONUS_VALUATION_DATE_LIE][t]=1;
			}
			if ((this->Assumption->Ratchet_Y_N==1)&&(t==this->ModelPoint->Calc_Deferal_Duration_Months)&&((int)ConstVector[IND_CONTRACT_PERIOD_LIE][t]==1) )
			...
			ConstVector[IND_DB_PERIOD_LIE][t]=ConstVector[IND_CONTRACT_PERIOD_LIE][t]*(this->ModelPoint->Calc_GMDB_Y_N ==1)*(this->Assumption->Max_Age_Mortality > ConstVector[AGE_LIE][t])*(this->Assumption->Max_Age_DB > ConstVector[AGE_LIE][t]);

		}
	}
void Projection_BL_LIE_DEIT_GMWB_01::Calculate_ConstVector_decrement()
	{
	...
	//////////////////////////////////////////////////////////////////////////////////
void Projection_BL_LIE_DEIT_GMWB_01::projection_base_contract(int curr_shock_number)
	{
		set_Local_Parameters_to_zero();
		...
		for( int t=0; t <= this->ModelPoint->Calc_Remaining_Months ; t++)
		{
			...
			else
			{
				...
				bonus_value=Performance_bonus_valuation*ConstVector[IND_BONUS_VALUATION_DATE_LIE][t-1];
				...
				claims_wb_t=max(0.,annuity_per_freq*ConstVector[IND_COUPON_DATE_LIE][t]+bonus_value-account_value);	
				...
				account_value-=min(account_value,((annuity_per_freq+add_coupon_from_ratchet*ConstVector[IND_RATCHET_MONTH_LIE][t-1]))*ConstVector[IND_COUPON_DATE_LIE][t]);
				//}
				//else 
				//	claims_wb_t=0.;
				...
				add_coupon_from_ratchet=ConstVector[IND_RATCHET_MONTH_LIE][t]*(  max( account_value/this->ModelPoint->Calc_NB_Coupons,annuity_per_freq ) - annuity_per_freq );
				if((int)ConstVector[IND_RATCHET_MONTH_LIE][t]==1)
			...
			if(myGenerateDebugFiles) //If debuging the porjection : not used in production
			{
				...
				this->set_value_on_debug_file(this->basis_capital_1,BASIS_KAPITAL_LIE, t);
				...
				this->set_value_on_debug_file(this->Performance_bonus_valuation,PERFORMANCE_BONUS_VAL_LIE, t);	
				this->set_value_on_debug_file(this->bonus_value,BONUS_VAL_LIE, t);
				...
				this->set_value_on_debug_file(this->mort_val,MORT_VAL_LIE, t);
...
void Projection_BL_LIE_DEIT_GMWB_01::PresentValueCalculation(int curr_shock_number,bool delta)
	{
		...
		// Calculate rates
		//this->ModelPoint->index_shock_pvs[curr_shock_number][PV_RRC_RATE_TOT_BL]=this->ModelPoint->Ong_Charge_1_Bl ;				
		...
		this->ModelPoint->index_shock_pvs[curr_shock_number][PV_ONGOING_COMM_RATE]=0.;												//this->ModelPoint->Ong_Acqui_Charge_Lux;
		this->ModelPoint->index_shock_pvs[curr_shock_number][PV_TAIL_COMM_RATE]=0.;												//this->ModelPoint->Trail_Comm_Lux;
		this->ModelPoint->index_shock_pvs[curr_shock_number][PV_EHC_NEW_RE_RATE]=this->ModelPoint->New_Re_Reinsurance_premium;												//this->ModelPoint->Reinsurance_Premium_Lux;
