#include "Projection_BL_LUX_WBLD_01.h"
...
Assumption_BL_LUX_GMWBLD_01::~Assumption_BL_LUX_GMWBLD_01(){ delete this->Longevity_mortality_table;}
Assumption_BL_LUX_GMWBLD_01::Assumption_BL_LUX_GMWBLD_01():Assumption_BL_LUX_GMWBD_01(){}
Assumption_BL_LUX_GMWBLD_01::Assumption_BL_LUX_GMWBLD_01(const char * tables):Assumption_BL_LUX_GMWBD_01(tables)
{
	// Assumptions tables------------------------------------------------
	map<string, GMXB_Assumption_Table*> sheet_tables;
	sheet_tables["Longevity_mortality_table"] = 0;
	Longevity_mortality_table=sheet_tables["Longevity_mortality_table"];
	Longevity_Y_N= atoi(this->assump->search("Longevity_Y_N").c_str());
}
double Assumption_BL_LUX_GMWBLD_01::Get_mortality_rates_vector(const string& sex,const int& current_year,const int& valn_age,const int& valn_year)
{
	this->index_1_local=(sex == "MALE")*(current_year-2007)+(sex != "MALE")*(current_year-2007+88);
...
double Assumption_BL_LUX_GMWBLD_01::Get_longevity_mortality_rates_vector(const string& sex,const int& current_year,const int& valn_age,const int& valn_year)
{
	this->index_1_local=(sex == "MALE")*(current_year-2007)+(sex != "MALE")*(current_year-2007+88);
	this->index_2_local=valn_age-29-valn_year+2008;
	this->mort_value_local=this->Longevity_mortality_table->search(this->index_1_local,this->index_2_local);
	this->mort_value_local=1.-exp(log(1.-this->mort_value_local)/12.);
	return this->mort_value_local;
}	
...
Projection_BL_LUX_GMWBLD_01::~Projection_BL_LUX_GMWBLD_01(){	delete this->Assumption;}
Projection_BL_LUX_GMWBLD_01::Projection_BL_LUX_GMWBLD_01( const vector<vector <int>>& i_s_a,const char * tables, bool generate_dbg_files,const string& debug_file_path)
	:Projection(i_s_a,tables,generate_dbg_files,debug_file_path)
{
	this->Assumption =new Assumption_BL_LUX_GMWBLD_01(tables);
	...
	///////////////////////////////////////////////////////////////////////////////////
	//////////////////////////////////////////////////////////////////////////////////
	void Projection_BL_LUX_GMWBLD_01::mySet_Debug_File_Header(){
	...
	void Projection_BL_LUX_GMWBLD_01::set_current_model_point(Model_Point *mp)
	{
	...
	void Projection_BL_LUX_GMWBLD_01::initialise_mp_calculation()
	{
		...
		this->ModelPoint->Calc_WB_Rollup=this->ModelPoint->Benefit_Increase_Factor*(1. + this->ModelPoint->Rollup_Rate_Lux*this->ModelPoint->Calc_Initial_Deferal_Months/12.)*this->ModelPoint->Premium;
		
		this->ModelPoint->Calc_Initial_Coupon_by_Freq=this->ModelPoint->Calc_WB_Rollup*this->ModelPoint->Withdrawal_Rate/this->ModelPoint->Calc_Freq_Pay/(1.+this->ModelPoint->Withrawals_Mode_Discount_Lux);
	...
	void Projection_BL_LUX_GMWBLD_01::Calculate_ConstVector_guarantees()
	{
		for( int t=0; t <= this->ModelPoint->Calc_Remaining_Months ; t++)
		{
			...
			else if ((this->Assumption->Ratchet_Y_N==1)&&(t==this->ModelPoint->Calc_Deferal_Duration_Months)&&((int)ConstVector[IND_CONTRACT_PERIOD_BL_LUX][t]==1) )
				ConstVector[IND_RATCHET_MONTH_BL_LUX][t]=1;
	...
	void Projection_BL_LUX_GMWBLD_01::Calculate_ConstVector_decrement()
	{
		...
		if(this->Assumption->Mortality_Y_N==1)
		{
			...
			{
				if(indic_1!=0)
					qx_month_1_local=this->Assumption->Get_mortality_rates_vector(this->ModelPoint->Gender_1_Lux,
											(int)ConstVector[DATE_YEAR_BL_LUX][t],this->ModelPoint->Calc_Age_1,this->ModelPoint->Calc_Valn_Year);
				if(indic_2!=0)
					qx_month_2_local=this->Assumption->Get_mortality_rates_vector(this->ModelPoint->Gender_2_Lux,
											(int)ConstVector[DATE_YEAR_BL_LUX][t],this->ModelPoint->Calc_Age_2,this->ModelPoint->Calc_Valn_Year);
				...
				{
					if(indic_1!=0)
						qx_month_1_local_longevity=this->Assumption->Get_longevity_mortality_rates_vector(this->ModelPoint->Gender_1_Lux,
											(int)ConstVector[DATE_YEAR_BL_LUX][t],this->ModelPoint->Calc_Age_1,this->ModelPoint->Calc_Valn_Year);
					if(indic_2!=0)
						qx_month_2_local_longevity=this->Assumption->Get_longevity_mortality_rates_vector(this->ModelPoint->Gender_2_Lux,
											(int)ConstVector[DATE_YEAR_BL_LUX][t],this->ModelPoint->Calc_Age_2,this->ModelPoint->Calc_Valn_Year);
					...
					if(alle_verbleibend_t_bef_longevity!=0.)
						ConstVector[DEATH_R_BL_LUX][t]=local_Alle_Tote_longevity/alle_verbleibend_t_bef_longevity;
					else
						ConstVector[DEATH_R_BL_LUX][t]=1.;
	...
	//////////////////////////////////////////////////////////////////////////////////
	void Projection_BL_LUX_GMWBLD_01::projection_base_contract(int curr_shock_number)
	{
		...
		for( int t=0; t <= this->ModelPoint->Calc_Remaining_Months ; t++)
		{
			...
			else
			{
				...
				if ((int)ConstVector[IND_COUPON_DATE_BL_LUX][t]==1)
					basis_capital=max(basis_capital-annuity_per_freq-add_coupon_from_ratchet,0.)*survival_index_by_step;
				else
				...
				///////////////////Additional coupon from ratchet + Basis Capital...
				if((int)ConstVector[IND_RATCHET_MONTH_BL_LUX][t]==1)
				{
					if (this->ModelPoint->Calc_Initial_Deferal_Months==0)
						add_coupon_from_ratchet=this->ModelPoint->Benefit_Increase_Factor * max(0.,account_value-basis_capital)
								*this->ModelPoint->Withdrawal_Rate/this->ModelPoint->Calc_Freq_Pay/(1.+this->ModelPoint->Withrawals_Mode_Discount_Lux);
					else
						add_coupon_from_ratchet=max(0.,account_value-basis_capital)
								*this->ModelPoint->Withdrawal_Rate/this->ModelPoint->Calc_Freq_Pay/(1.+this->ModelPoint->Withrawals_Mode_Discount_Lux);
					
					basis_capital=max(basis_capital,account_value);
				}
				else
					add_coupon_from_ratchet=0.;
				///////////////////End Additional coupon from ratchet + Basis Capital...
	...
	void Projection_BL_LUX_GMWBLD_01::PresentValueCalculation(int curr_shock_number,bool delta)
	{
