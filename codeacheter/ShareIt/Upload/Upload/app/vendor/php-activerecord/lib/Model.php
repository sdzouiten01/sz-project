...
class Model
{
	...
	public function assign_attribute($name, $value, $cast = true)
	{
		...
		if (array_key_exists($name,$table->columns) && !is_object($value)) {
		  //checks if $cast is enabled, if so returned cast() value
		  if($cast === true)
	...
	/**
	 * Unsets (Remove) an attribute from the model.
	 *
	 * @param string $name Name of the attribute
	 */
	 ...
	 /**
	  * Unsets (Remove) an attribute list from the model.
	  *
	  * @param string $array Name of the attributes
	  */
	  public function unset_attributes($names) {
	 ...
	 */
	public function update_attributes($attributes, $guard_attributes = true)
	{
		$this->set_attributes($attributes, $guard_attributes);
		return $this->save();
	}
	...
	/**
	 * Mass refreshs the model's attributes without saving them to db
	 */
	public function refresh_attributes()
	{
	  foreach ($this->attributes as $attribute => $value) {
	    // check for getter
	    if (in_array("get_$attribute",static::$getters))
	    {
	    	$name = "get_$attribute";
	    	$value = $this->$name();
	 ...
	 */
	public function set_attributes(array $attributes, $guard_attributes = true)
	{
		$this->set_attributes_via_mass_assignment($attributes, $guard_attributes);
	}
	...
	private function serialize($type, $options)
	{
		...
		//refreshses attributes by calling it's getters
