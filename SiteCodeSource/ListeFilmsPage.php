...
<html>
    ...
    <!--[if !IE]><!--><body><!--<![endif]-->
		...
		<?php	
			if(isset($_GET['TITRE']))
			{
				echo '<p>Titre         	: <input type="text" name="TITRE" value="' . $_GET['TITRE'] . '">';
			}else{
				echo '<p>Titre         	: <input type="text" name="TITRE">';
			}
			if(isset($_GET['REALI']))
			{
				echo ' Réalisateur         	: <input type="text" name="REALI" value="' . $_GET['REALI'] . '">';
			}else{
				echo ' Réalisateur         	: <input type="text" name="REALI">';
			}
			echo ' Genre   		: ';
			if(isset($_GET['GENRE']))
			{
		...
		?>			
					...
					<input type="submit" value="LookForIt" class="bouton_rouge" /></p>
				</form></pcherche>
	...
	<section>
			...
			while ($donnees = $reponse->fetch())
			{
				...
				$NBPAGE = ceil((int)$ALLFILMNB/$NBFILMBYPAGE);
		...
		echo'<a class="page" title="' . $i .'" href="ListeFilmsPage.php?currentpagenb='.$i.'&RechecheWidget=1' . $STRGENRE . $STRTITRE . $STRREALI.'">' . ' ' . $i . ' ' .'</a>';
			...
			$QueryShow= $Query . ' LIMIT ' . ($currentpagenb-1)*$NBFILMBYPAGE . ', ' . $currentpagenb*$NBFILMBYPAGE;
		    	$reponse = $bdd->query($QueryShow);
			$IDORDER = 1;
		    while ($donnees = $reponse->fetch())
			{
			
				$letterstoreplace = array('+');
				$withletters   = array('_');
				$textLinkFilm    = 'video/' . $donnees['NAMEFILM'] . '.m4v';
				$output  = str_replace($letterstoreplace, $withletters, $textLinkFilm);
